...
import { switchEnvironment } from "../lib/api";
import type {
  Environment,
  ProjectDetail,
  RepoStatus,
...
export function EnvironmentSwitcher() {
  ...
  const initialPreview = peekSwitchPreview(
    projectId,
  ...
  const [envId, setEnvId] = useState<string | null>(initialEnvironmentId);
  const [branchMap, setBranchMap] = useState<Record<string, string>>(initialBranchMap ?? {});
  const [statuses, setStatuses] = useState<Record<string, RepoStatus>>(initialWorkspace?.statuses ?? {});
  const [preview, setPreview] = useState<Record<string, SwitchPreviewItem>>(initialPreview ?? {});
  ...
  const refresh = useCallback(async (force = true) => {
    try {
      ...
      setDetail(workspace.detail);
      setEnvs(workspace.environments);
      setEnvId((prev) => {
        if (prev && workspace.environments.some((x) => x.id === prev)) return prev;
        return workspace.environments.find((x) => x.isDefault)?.id ?? workspace.environments[0]?.id ?? null;
      });
  ...
  useLayoutEffect(() => {
    ...
    const cached = peekEnvironmentBranchMap(envId);
    if (cached) {
      setBranchMap(cached);
      return;
    }
    ...
    void (async () => {
      try {
        const map = await loadEnvironmentBranchMap(envId);
  ...
  useLayoutEffect(() => {
    ...
    void (async () => {
      try {
        const items = await loadSwitchPreview(projectId, envId, options);
        if (cancelled) return;
        setPreview(items);
      } catch (err) {
        if (!cancelled) setToast({ msg: String(err), error: true });
  ...
  return (
    <>
      ...
      {contentLoading ? (
        <PageLoading label="Loading environment preview…" />
      ) : (
        <>
